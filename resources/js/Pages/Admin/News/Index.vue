...
watch([search, status], ([s, st]) => {
  ...
  if (st === 'trashed') {
    router.get(route('admin.news.trash'), params, { preserveState: true, replace: true });
    return;
  }
  if (st) params.status = st;
  router.get(route('admin.news.index'), params, { preserveState: true, replace: true });
});
</script>
  ...
  <AuthenticatedLayout>
    <template #header>
      <div class="flex items-center justify-between">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">News</h2>
        <Link :href="route('admin.news.create')" class="inline-flex items-center rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">Create News</Link>
      </div>
    </template>
            ...
            <table class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gray-50">
                <tr>
                  ...
                  <th class="px-3 py-2 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Featured</th>
              ...
              <tbody class="divide-y divide-gray-200 bg-white">
                <tr v-for="item in news.data" :key="item.id">
                  ...
                  <td class="px-3 py-2 text-sm text-gray-700">{{ item.status }}</td>
                  <td class="px-3 py-2 text-sm text-gray-700">{{ item.visibility }}</td>
                  <td class="px-3 py-2 text-sm text-gray-700">{{ item.published_at || '-' }}</td>
                  <td class="px-3 py-2 text-sm text-gray-700">{{ item.is_featured ? 'Yes' : 'No' }}</td>
                  <td class="px-3 py-2 text-right text-sm space-x-2">
                    <Link v-if="item.public_url" :href="item.public_url" target="_blank" class="text-gray-700 hover:text-gray-900">View</Link>
                    <Link :href="route('admin.news.edit', item.id)" class="text-indigo-600 hover:text-indigo-900">Edit</Link>
                    <button @click="router.delete(route('admin.news.destroy', item.id), { preserveScroll: true })" class="text-red-600 hover:text-red-800">Trash</button>
                  </td>
                </tr>
              </tbody>
            </table>
