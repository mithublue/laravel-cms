  ...
  <AuthenticatedLayout>
    <template #header>
      <div class="flex items-center justify-between">
        ...
        <Link :href="route('admin.users.create')" class="inline-flex items-center rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">Add New User</Link>
      </div>
    </template>
    ...
    <div class="py-6">
      <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
        <div class="bg-white p-4 shadow sm:rounded-lg">
          <div class="mb-4">
            <input v-model="search" type="text" placeholder="Search users..." class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
          </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gray-50">
                <tr>
                  ...
                  <th class="px-3 py-2 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Roles</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-200 bg-white">
                <tr v-for="user in users.data" :key="user.id">
                  ...
                  <td class="px-3 py-2 text-sm text-gray-700">{{ (user.roles || []).join(', ') }}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div class="mt-4 flex flex-wrap items-center gap-2">
            <Link v-for="link in users.links" :key="link.url + link.label" :href="link.url || '#'" preserve-scroll :class="[
